...
{
  /* USER CODE BEGIN 13 */
	if( BSP_QSPI_FastRead(buf,blk_addr*STORAGE_BLK_SIZ,STORAGE_BLK_SIZ*blk_len) != 0)
		return (USBD_FAIL);
	return (USBD_OK);
  /* USER CODE END 13 */
}
...
{
  /* USER CODE BEGIN 14 */
	if( BSP_QSPI_Erase_Block(blk_addr*STORAGE_BLK_SIZ) != 0)
		return (USBD_FAIL); 
	if( BSP_QSPI_Write(buf,blk_addr*STORAGE_BLK_SIZ,STORAGE_BLK_SIZ*blk_len) != 0)
		return (USBD_FAIL);
	return (USBD_OK);
  /* USER CODE END 14 */
}
